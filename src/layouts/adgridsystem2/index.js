...
function AdGridSystem2() {
  const [controller] = useMaterialUIController();
  const { darkMode } = controller;
  const { gridItems, loading, error, fetchAllGridItems, reorderGridItems } = useAdGrid2();
  ...
  const filteredItems = useMemo(() => {
    ...
    return gridItems.filter(
      (item) =>
        ...
        item.buttonText?.toLowerCase().includes(lowerSearch)
    );
