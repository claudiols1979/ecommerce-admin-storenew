...
export const useAdGrid2 = () => useContext(AdGrid2Context);
...
export const AdGrid2Provider = ({ children }) => {
  ...
  const fetchGridItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await axios.get(`${API_URL}/api/ad-grid-2/public`);
      setGridItems(data);
      return data;
    } catch (err) {
  ...
  const fetchAllGridItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await axios.get(`${API_URL}/api/ad-grid-2`, config);
      setGridItems(data);
      return data;
    } catch (err) {
      const errorMessage = err.response?.data?.message || "Error loading grid items";
  ...
  const fetchGridItemById = useCallback(
    async (id) => {
      ...
      setError(null);
      try {
        const { data } = await axios.get(`${API_URL}/api/ad-grid-2/${id}`, config);
        setCurrentItem(data);
        return data;
      } catch (err) {
  ...
  const createGridItem = useCallback(
    async (itemData, imageFile) => {
      ...
      try {
        ...
        const { data } = await axios.post(`${API_URL}/api/ad-grid-2`, formData, {
          ...config,
          headers: {
            ...config.headers,
  ...
  const updateGridItem = useCallback(
    async (id, itemData, imageFile) => {
      ...
      try {
        ...
        const { data } = await axios.put(`${API_URL}/api/ad-grid-2/${id}`, formData, {
          ...config,
          headers: {
            ...config.headers,
            "Content-Type": "multipart/form-data",
  ...
  const deleteGridItem = useCallback(
    async (id) => {
      ...
      setError(null);
      try {
        await axios.delete(`${API_URL}/api/ad-grid-2/${id}`, config);
    ...
    async (itemsOrder) => {
      ...
      try {
        const { data } = await axios.put(
          `${API_URL}/api/ad-grid-2/reorder/reorder`,
          {
            items: itemsOrder,
          },
          config
  ...
  const bulkUpdateStatus = useCallback(
    async (ids, isActive) => {
      ...
      try {
        const { data } = await axios.put(
          `${API_URL}/api/ad-grid-2/bulk/status`,
          {
            ids,
            isActive,
          },
  ...
  return <AdGrid2Context.Provider value={value}>{children}</AdGrid2Context.Provider>;
};

AdGrid2Provider.propTypes = {
